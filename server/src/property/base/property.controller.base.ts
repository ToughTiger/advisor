...
export class PropertyControllerBase {
  ...
  async findManyConfiguration(
    ...
    const results = await this.service.findConfiguration(params.id, {
      where: query,
      select: {
        bhktype: true,
        createdAt: true,
  ...
  @common.UseInterceptors(nestMorgan.MorganInterceptor("combined"))
  @common.UseGuards(basicAuthGuard.BasicAuthGuard, nestAccessControl.ACGuard)
  @common.Delete("/:id/configuration")
  @nestAccessControl.UseRoles({
    resource: "Property",
    action: "update",
    possession: "any",
